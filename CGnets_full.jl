...
begin
	...
	traj = [rand(Float32, 2) .* 10f0 .- 5f0 for _ in 1:n_samples]
...
# ╔═╡ abfcbcce-f022-481c-bce9-e5cb08cbb6dd
function force_from_potential(xb, model)
    # We assume xb is of shape (1, N), a 1D input with N examples
    function energy(x)
        sum(model(x))  # Zygote works best with scalar outputs
    end
    ∇E = Zygote.gradient(energy, xb)[1]  # shape should match xb
    return -∇E  # force = negative gradient
end
...
CGnet = gpu(CGnet)  # Force model and weights to GPU
end

# ╔═╡ 568d506d-3b0a-4aa2-ae85-489ceaf057c2
begin
	test_x = gpu(reshape(Float32.(-2.0:0.1:2.0), 1, :))
	@show size(test_x)
	f_test = force_from_potential(test_x, CGnet)
	@show size(f_test)  # should match size(test_x)
end
...
function batch_loss_fast_gpu(xb, fb, model)
    f_pred = force_from_potential(xb, model)
...
function train_cgnet(model, x_data, f_data; num_epochs=50, batchsize=256, learning_rate=0.000001)
    ...
    for epoch in 1:num_epochs
        ...
        for i in 1:batchsize:length(inds)
            ...
            xb_cpu = collect(xb)  # or use Array(xb)
            println("xb range: min = $(minimum(xb_cpu)), max = $(maximum(xb_cpu))")
