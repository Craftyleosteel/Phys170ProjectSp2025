...
# ╔═╡ 55e3cbbe-3393-4f5a-b1d8-ef5348575f0c
begin
	CUDA.has_cuda()
	CUDA.functional()
end

# ╔═╡ d45b0504-2253-4edb-a592-b09c8b5e10fd
CUDA.versioninfo()
...
# ╔═╡ f9510cb9-369e-4e3f-80fd-056f3ae4092b
function batch_loss_safe(model, b_gpu, fb_gpu)
    loss, back = Zygote.pullback(x -> sum(model(x)), xb)
    f_pred = -back(1f0)[1]
...
# ╔═╡ 2f713831-e9bd-423d-880e-6f930e5db019
function loss_fn(params, model, xb, fb)
    function energy(x)
        sum(model(x))
    end
    f_pred = -Zygote.gradient(energy, xb)[1]
    return mean((f_pred .- fb).^2)
end
...
begin
    ...
    opt_state = Optimisers.setup(opt, Flux.trainable(CGnet))
    ...
    for epoch in 1:num_epochs
        epoch_loss = 0.0
        inds = shuffle(1:size(x_train, 1))

        for i in 1:batchsize:length(inds)
            idx = inds[i:min(i + batchsize - 1, end)]
            xb = reshape(x_train[idx], 1, :)
            fb = f_train[idx]

            grads = Zygote.gradient(Flux.trainable(CGnet)) do p
                loss_fn(p, CGnet, xb, fb)
            end

            opt_state = Optimisers.update!(opt_state, Flux.trainable(CGnet), grads)
            epoch_loss += loss_fn(Flux.trainable(CGnet), CGnet, xb, fb)
        end
...
manifest_format = "2.0"
project_hash = "9eaf1753dbb1c70aa21c47bd4a7560ba1ae08411"
