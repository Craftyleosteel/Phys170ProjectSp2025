...
function update_model!(model, modelstate, gamma, buffer)
    batch = rand(buffer, min(50, length(buffer)))
    states, actions, rewards, next_states = first.(batch), getindex.(batch, 2), getindex.(batch, 3), getindex.(batch, 4)
    
    q_values = model(hcat(states...))
    next_q_values = model(hcat(next_states...))
