...
# ╔═╡ 14b61e5e-19b5-11f0-12e9-f7aa8bed6eab
using Markdown

# ╔═╡ a6a74f29-357d-4ed1-a6b6-501159a484dc
using InteractiveUtils

# ╔═╡ 4d92b909-23fc-4535-9df3-e054b87cf24e
begin
	using Flux
	using Zygote
	using LinearAlgebra
...
# ╔═╡ b03a1e95-e5a1-43e9-8346-0855a065cce0
md"""Adjustments to CGNets code.
"""

# ╔═╡ db314da3-08fc-4315-bed4-92b3502c2634
md"""
Toy potential function.
"""

# ╔═╡ 9b477850-e299-4a4c-b5ac-1766178816fe
function V(x, y)
...
# ╔═╡ 9a112ddd-2cbc-4625-ae04-52ccade7f32b
function simulate_trajectory(T, τ, D, seed)
	Random.seed!(seed)
	x, y = 0.0, 0.0
	traj = Matrix{Float64}(undef, T, 2)  # changed

	for t in 1:T
		∂Vx = Zygote.gradient(z -> V(z, y), x)[1]  # changed
		∂Vy = Zygote.gradient(z -> V(x, z), y)[1]  # changed
...
# ╔═╡ 821f11c8-ea36-428b-9d0c-1e60f7ddd0dd
md"""
Generate trajectories.
"""

# ╔═╡ 7df88910-cbd7-421b-b81f-809807794e3f
begin
	T = 10_000
	τ = 0.01
	D = 1.0
...
# ╔═╡ b488bff2-a58d-4058-b495-bd5f9a3632eb
begin
	all_trajs = ThreadsX.map(i -> simulate_trajectory(T, τ, D, 42 + i), seeds)  # changed
	traj = vcat(all_trajs...)
end

# ╔═╡ ed852e10-fd55-43fc-9667-10d2d02d16c2
md"""
Compute force field.
"""

# ╔═╡ fb7435ea-f3fc-4797-8a40-68e517d1b605
function compute_instantaneous_forces(traj)
	n = size(traj, 1)
	xs = traj[:, 1]
	ys = traj[:, 2]
	forces = zeros(n)  # changed

	for i in 1:n
		forces[i] = -Zygote.gradient(z -> V(z, ys[i]), xs[i])[1]  # changed
	end

	return xs, forces
end

# ╔═╡ 3649cc2e-fbfd-4523-a9bc-585fa7fd2160
x_vals, fx_vals = compute_instantaneous_forces(traj)

# ╔═╡ ae5a3d85-f30f-4f66-a697-40c94c19a3ef
md"""
Define neural network (CGNet).
"""

# ╔═╡ 495083da-9260-4c2c-8371-0cd9f9b8d896
CGnet = Chain(
	Dense(1, 50, tanh),
	Dense(50, 50, tanh),
...
# ╔═╡ aec2af7b-5950-492a-8757-28ef0b66ef7a
function predict_force(x)
    U(x) = CGnet(Float32[x])[1]
    -Zygote.gradient(U, x)[1]
end

# ╔═╡ eef8f9a2-b075-431b-8f69-e43d009eec13
md"""
Loss function.
"""

# ╔═╡ 54e3fb95-9408-4871-9319-9eca7167caf5
function loss_fn(xs, fs)
    preds = map(predict_force, xs)
    return mean((preds .- fs).^2)
end

# ╔═╡ 7f9dde85-cc9d-4803-8ab4-25f9578f9c44
begin
	opt = Optimisers.Adam(0.01)
	state = Optimisers.setup(opt, CGnet)
end

# ╔═╡ 9e976d9a-bb63-4694-8662-3618c660ef85
begin
	loss_history = Float64[]
	num_epochs = 50
end

# ╔═╡ 320056bd-8304-40cb-a508-5d69081708ae
@showprogress 1 "Training CGnet..." for epoch in 1:num_epochs
	...
	push!(loss_history, loss_fn(x_vals, fx_vals))
end
...
manifest_format = "2.0"
project_hash = "91a8ff52048efc56e74d79080266bd7f74e7931b"
