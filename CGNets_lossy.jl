...
begin
	# Generate simple data points spread evenly
    n_samples = 1000
    Random.seed!(123)
    
    # Simple linear data generation (x from -2 to 2)
    x_vals = collect(range(-2.0f0, 2.0f0, length=n_samples))
    
    # The force is -x for harmonic potential
    f_vals = -x_vals
    
    # Add a tiny bit of noise to make it more realistic
    f_vals = f_vals .+ 0.01f0 .* randn(Float32, n_samples)
end
...
x_train = reshape(Float32.(x_vals), 1, :)
f_train = Float32.(f_vals)
end
...
# n_total = size(x_train_gpu, 2)
# n_train = Int(floor(0.9 * n_total))
# n_val = n_total - n_train

# # Use consecutive indices to avoid data leakage issues
# train_indices = 1:n_train
# val_indices = (n_train+1):n_total

# # Split data
# x_train_data = x_train_gpu[:, train_indices]
# f_train_data = f_train_gpu[train_indices]
# x_val_data = x_train_gpu[:, val_indices]
# f_val_data = f_train_gpu[val_indices]
# end
...
# ╔═╡ 168c21d9-f087-4a4e-8106-a6e4ebb28372
# begin
# # Improved architecture with more layers and different activation functions
# CGnet = Chain(
#     # Input normalization layer
#     Dense(1, 128, tanh; init=Flux.glorot_uniform),  # Larger first layer, tanh activation
...
#     loss = mean((f_pred .- fb).^2)
# 	return isnan(loss) ? 1000.0f0 : loss
# end


# ╔═╡ 4af42497-2b03-45fc-9c07-b56e9b8255bf
function loss_fn(x, y)
    pred = CGnet(x)
    return Flux.mse(pred, reshape(y, 1, :))
end

# ╔═╡ 384edc31-937b-4974-a28e-66320113ccd5
function validate(model, x_val, f_val)
    return batch_loss_fast_gpu(x_val, f_val, model)
end
...
# ╔═╡ a14898a0-62fc-44e9-a3f8-9b7aa36611b1
begin
# Basic training loop
function train_simple(epochs=100)
    # Create optimizer
    opt = Flux.setup(Flux.Adam(0.01), CGnet)
    ...
    for epoch in 1:epochs
        # Create batches
        data = Flux.DataLoader((x_train, f_train), batchsize=batch_size, shuffle=true)
        
        # Train on batches
        batch_losses = []
        for (x_batch, f_batch) in data
            # Compute gradients
            val, grads = Flux.withgradient(m -> loss_fn(x_batch, f_batch), CGnet)
            
            # Update parameters
            Flux.update!(opt, CGnet, grads[1])
...
# ╔═╡ 6de58dd9-2159-4b88-9a1c-dc7d2c740ac6
begin
	# Train model
	loss_history = train_simple(1000)
end

# ╔═╡ c6272257-e66c-494d-bb41-c787cf591bdc
begin
    # Plot training loss
    plot(loss_history, 
        xlabel="Epoch", 
...
# ╔═╡ bb1a4b07-bbec-4f4d-b641-17f3ea1f9b02
begin
    # Generate test data spanning a wider range than training
    x_test = collect(-4.0f0:0.1f0:4.0f0)
    x_test_input = reshape(x_test, 1, :)
    
    # Get model predictions
    y_pred = CGnet(x_test_input)[1, :]
    
    # Calculate true forces
    y_true = -x_test
    
    # Calculate MSE
    test_mse = mean((y_pred .- y_true).^2)
    
    # Plot results
    p1 = plot(x_test, y_true, label="True Force (F = -x)", lw=2)
    plot!(p1, x_test, y_pred, label="CGNet Prediction", lw=2, ls=:dash)
    xlabel!(p1, "Position (x)")
    ylabel!(p1, "Force")
    title!(p1, "Force Prediction")
    
    # Plot error
    p2 = plot(x_test, abs.(y_pred .- y_true), 
        label="Prediction Error", 
        lw=2,
        xlabel="Position (x)", 
        ylabel="Error",
        title="Error: MSE = $(round(test_mse, digits=6))")
...
# ╔═╡ a1b4a1d2-32be-4f3f-aa4b-6e1dd040f612
# begin
#     p1 = plot(
#         train_loss_history, 
#         xlabel="Epoch", 
...
#         lw=2,
#         yscale=:log10  # Log scale to better see convergence
#     )
#     plot!(p1, val_loss_history, label="Validation Loss", lw=2, ls=:dash)
    
#     # Add log loss ratio plot
#     p2 = plot(
#         train_loss_history ./ first(train_loss_history),
#         xlabel="Epoch",
#         ylabel="Loss Ratio (log scale)",
#         label="Training Loss Ratio",
#         title="Loss Ratio (normalized to first epoch)",
#         lw=2,
#         yscale=:log10
#     )
...
# ╔═╡ 94315675-c38a-4cec-b7f5-31b34c79c4dc
# begin
#     # Define a broader range to test extrapolation capabilities
#     x_test = collect(-4f0:0.01f0:4f0)
#     x_test_gpu = reshape(gpu(x_test), 1, :)

#     f_pred = trained_model(x_test_gpu)[1, :] |> collect
#     f_true = -x_test

#     # Calculate mean squared error on test data
#     mse = mean((f_pred .- f_true).^2)
    
#     # Plot with error information
#     p = plot(
#         x_test, f_true, 
#         label="True F = -x", 
#         lw=2,
...
#         xlabel="Position (x)",
#         ylabel="Error",
#         title="Prediction Error: MSE = $(round(mse, digits=8))"
#     )
    
#     plot(p, error_plot, layout=(2, 1), size=(800, 600))
# end
