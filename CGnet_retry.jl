...
# ╔═╡ 72de56fe-2682-11f0-3983-b33b042a248b
begin
	using Flux
	using Zygote
	using Random
...
# ╔═╡ 4acda57f-6169-4f60-9115-0a66f141c7eb
md"""Functions to define toy potential and Langevin dynamics of system.
"""

# ╔═╡ 18799a6c-a66c-4a18-9135-a02296730fb8
begin
	# Define toy potential.
	function V(x, y)
...
# ╔═╡ c4dec147-803b-4730-9337-09fa06cbddb9
md"""Generate training data.
"""

# ╔═╡ 8f1d95a2-c841-4879-8abb-f713ccb889f5
begin
	T = 10_000
	τ = 0.01
	D = 1.0
	seeds = 1:50
	all_trajs = ThreadsX.map(i -> simulate_trajectory(T, τ, D, 42 + i), seeds)
	traj = vcat(all_trajs...)
end

# ╔═╡ 05947518-2ebb-4dea-82b8-4f0433ce8eaa
md"""Extract forces.
"""

# ╔═╡ 0d79bf4e-70bb-4ea2-a3e7-7397e502fd36
begin
	function compute_instantaneous_forces(traj)
	    ...
	    forces = zeros(n)
	    for i in 1:n
	        forces[i] = -Zygote.gradient(z -> V(z, ys[i]), xs[i])[1]
	    end
	    return xs, forces
	end
end

# ╔═╡ 16fe1bc1-30c2-4aef-8cb9-8026c6bed780
md"""Normalize input."""

# ╔═╡ 2b9de886-351c-4655-b38d-48feffd40786
begin
	x_vals, fx_vals = compute_instantaneous_forces(traj)
	
	x_mean = mean(x_vals)
	x_std = std(x_vals)
	x_vals_norm = Float32.((x_vals .- x_mean) ./ x_std)
	fx_vals = Float32.(fx_vals)
end

# ╔═╡ 222723a7-2291-4fb7-a156-9541c14906fe
md"""Check GPU and define CGnet model."""

# ╔═╡ 5ed1951e-f716-4d1f-99ed-48c4bf2e6bc6
begin
	CUDA.has_cuda()  # should return true (and it does)
	CUDA.device()
end
...
manifest_format = "2.0"
project_hash = "c8a9cb2aca3a228e7655ade35e8889b8e116b5e1"
